...
es.onmessage=e=>{const{event,data}=JSON.parse(e.data);
 ...
 if(event==='song'){
   ...
   else if(data.phase==='rotate')line('l-dl',tag+'🔁 额度耗尽，已切换账号续跑');
 ...
 if(event==='quality')line(data.ok?'l-ok':'l-warn','🔎 '+(data.ok?('质检通过：'):('重排：'))+(data.title||'')+(data.reasons?('（'+data.reasons+'）'):''));
 if(event==='quality-done')line('l-mut','🔎 质量回捞：合格 '+data.good+' · 重排 '+data.requeued+' · 未捕获 '+data.unknown);
