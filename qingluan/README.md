...
npm run studio:smoke    # 65 项全链路冒烟测试（API + Agent + MCP stdio/HTTP）
```
...
1. **首页**：顶部「万能创作框」——一句话选模式（沉浸式短片 / 生成图片 / 短剧项目）+ 按次选
   视频模型、画幅、时长、分辨率，立即开始；下方仍有「AI 生剧本（集数/风格库）」「粘贴剧本」「自由画布」入口。
...
3. **节点画布**（`/#/canvas/:id`）：小云雀式深色无限画布——平移缩放、节点拖拽、端口连线
   ...
   生成分镜首帧时自动把连线的角色/场景定妆图传给 Seedream 当参考图（主体一致性）。
   「涂鸦」按钮（或 `D` 键）进入手绘批注：4 色画笔、3 档粗细、橡皮、清空，笔迹随画布保存、
   重新解析剧本也不丢，适合圈重点、画修改意见。
   快捷键：滚轮缩放、`F` 适配全部、`D` 涂鸦、`Delete` 删除选中、`⌘/Ctrl+S` 立即保存。
