...
npm run studio:smoke    # 41 项全链路冒烟测试（API + Agent + MCP stdio）
```
...
工作台「Agent 接入」页有可复制的现成命令与 Token。16 个开放工具覆盖全部能力：
`studio_overview / create_project / generate_script / write_script / parse_script / get_canvas /
update_node / generate_image / generate_video / generate_storyboard_media / get_task / list_assets /
import_asset / list_projects / get_project / get_usage_stats`。
...
- 端点：`POST /api/agent/v1/tools/{工具名}`，统一返回 `{ok, data:{result}}`
