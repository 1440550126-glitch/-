...
def col_splits(band, W):                    # 自相关求人物重复间距 -> 人数 -> 谷值强制切
    ...
    d = dens - dens.mean()
    n = 1
    if not np.allclose(d, 0) and cw > 40:
...
def cut_sheet(path):
    ...
    out_rows, boxes = [], []
    for (y0, y1) in row_bands(fg, W):
        rc = []
        for (x0, x1) in col_splits(fg[y0:y1], W):
            sub = fg[y0:y1, x0:x1]
