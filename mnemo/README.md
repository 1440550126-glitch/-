...
├─ tools.py        工具：文件/Shell/网页搜索+抓取/记忆/提醒/委派/视觉/语音；审批与拦截
...
```bash
python -m unittest discover -s tests -v   # 75 项全链路冒烟，全部通过
```
...
**逐字流式输出**、**用量计量与定价**、**知识摄入 RAG**、**联网检索解析（离线）**。
...
**知识库 RAG（ingest）**、**联网检索（web_search）**。
