...
| **双向交流** | 对讲模式（播主人真声）+ 快捷短语 + 8 个条件反射训练音位，硬件零新增 |
...
- **猫 → 人**：叫声意图识别，前面章节已实现，这个方向是成立的；
- **人 → 猫**：**不存在"把人话翻译成猫语"的技术**。市面上"对猫播放合成猫叫"的玩法是伪科学，而且有害——在猫的领地里突然出现陌生猫的声音，会引发警觉和应激。所以本设备明确**禁止播放任何猫叫声**。

但"人→猫"有三条被行为学验证过的有效通道：**主人的声音**（研究表明猫能从陌生人中分辨出主人的声音并做出反应）、**条件反射音**（巴甫洛夫式声音-事件绑定）、**语气语调**。围绕这三条，设计三种双向模式——全部复用现有硬件，BOM 零新增：

### 模式一：对讲模式（实时喊话）

App 按住说话 → 语音经 BLE 传到项圈扬声器播出，猫听到的是**主人的真声**。

- 技术上：16 kHz 语音用 ADPCM 压缩到约 16 kbps，BLE 5.0 实测带宽 ≥ 100 kbps，传输绰绰有余；固件在 BLE GATT 上加一个音频写入特征值，分包收齐后走 I2S₁ 播放；
- 距离：BLE 有效范围约 10 m（隔一堵墙），覆盖"人在家、猫在别的房间"这个最高频场景；
- 出门在外的远程喊话需要 Wi-Fi + 云中转，功耗和复杂度都上一个台阶，列为 P5 选配，不进主线。

### 模式二：快捷短语（一键播放主人录音）

主人预先录好常用句，App 上一键触发："回家吃饭了""不可以抓沙发""妈妈马上回来"。比对讲更快，音质更稳定（本地 Flash 播放，无传输环节）。

### 模式三：训练音通道（真正让猫"听懂词"的方式）⭐

这是唯一被验证可以给猫建立"词汇表"的方法——**条件反射**：

1. 在 App 里给一个事件绑定一个专属提示音（如"开饭"= 三声上行短哨音）；
2. 之后每次开饭前 App 一键播放这个音，坚持 2~3 周；
3. 条件反射建立后，这个声音对猫就有了**真实、稳定的含义**——它是猫真正"听得懂"的一个词。

设备预留 **8 个训练音位**（开饭 / 回窝 / 玩耍 / 出门 / 剪指甲……由主人自定义），训练进度在 App 里按次数打卡跟踪。诚实地说：这个模式练成之后的沟通效果，比任何"翻译"都扎实。

### 双向模式的动物福利护栏

- 禁止播放猫叫声或来源不明的音效（应激风险，固件层面不提供此能力）；
- 对讲/短语播放有 10 s 冷却，防止对猫连续声音轰炸；
- IMU 判定猫在睡眠时，对讲默认降为最小音量（可在 App 关闭此保护，但默认开）；
- 所有播放同样受 75 dB@10cm 音量上限和静音时段约束。
...
| 阶段 | 内容 | 周期 |
|---|---|---|
| P0 桌面验证 | 洞洞板飞线 ESP32-S3 + INMP441 + MAX98357A，**识别到猫叫直接播报语音**。✅ 固件代码已就绪：[`meowtalk/`](../../meowtalk/README.md)（含语音包生成、CatMeows 训练脚本） | 1~2 周 |
| P1 原型机 | 打 PCB、打印外壳（含声腔）、上项圈实测；自采本猫数据扩到 8 分类；调防自触发与冷却策略 | 3~4 周 |
| P2 语音库与体验 | 批量生成 300+ 条播报语音、多语音包分区；适应期音量曲线、静音时段 | 2 周 |
| P3 双向交流 + App 联调 | BLE 协议 + 小程序实时卡片、纠错回传、语音包 OTA；**对讲模式（BLE 音频分包）、快捷短语、训练音通道与打卡** | 3~4 周 |
| P4 个性化 | 纠错样本微调，续航优化到 2 天+，IPX4 淋水测试 | 持续 |
| P5 远程喊话（选配） | Wi-Fi + 云中转，出门在外也能对讲；需评估功耗代价 | 待定 |
