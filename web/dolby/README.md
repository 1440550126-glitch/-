...
**个性化 HRTF**：内置 HRTF 是通用的；可用 `dolby.setHRIR(audioBuffer)` 上传**自己的双耳脉冲响应**
（HRIR/BRIR，`decodeAudioData` 得到的 AudioBuffer）→ 耳机模式改走卷积双耳，定位更贴合个人；`clearHRIR()` 还原。
...
> 在主信号路径上，更"干净"；不支持/失败自动回退软削波。默认关闭。
