...
**个性化 HRTF**：内置 HRTF 是通用的。两种方式换成你自己的：
- `dolby.setHRIR(audioBuffer)` 直接上传一段双耳脉冲响应（HRIR/BRIR，`decodeAudioData` 的 AudioBuffer）；
- `dolby.loadHRIRSet(set)` 用 **HRIR 数据集**（`dolby-hrir.js` 的格式：方位 + 每方位左右耳 FIR），
  自动取最近的左前/右前 HRIR 组成 4 声道真立体声卷积 IR。耳机模式下生效，`clearHRIR()` 还原。

> ⚠️ **SOFA**：不内置 SOFA(HDF5) 二进制解析（庞大且难验证）；请用社区脚本把 SOFA 离线转成上述
> HRIR 集格式（或导出成 wav 直接 `setHRIR`）。
...
dolby.setLoudnessNorm(-14);    // 响度归一化（类 Dolby Volume）：拉到目标 LUFS；getLoudness() 读瞬时
dolby.measureLoudness(true);   // 开启积分测量；getIntegratedLoudness() 读门控积分响度（BS.1770/R128 风格）
...
>
> **前瞻限幅器（实验性）**：构造时传 `{ limiterWorklet: true }`，用 AudioWorklet 前瞻峰值限幅替换软削波，
> 在主信号路径上，更"干净"；`setLimiterParams({threshold,attack,release})` 可**真机调参**，
> `getLimiterReduction()` 读增益衰减(dB)；不支持/失败自动回退软削波，默认关闭。
>
> **积分响度精度**：开启 `worklet:true` 时，响度测量在音频线程按 **400ms 窗 / 100ms 跳（75% 重叠）**
> 取块，`getIntegratedLoudness()` 更接近 BS.1770/R128；未开 worklet 时为周期近似块。
