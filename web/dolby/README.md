...
- 🎚 **5 档预设** — 标准增强 / 影院环绕 / 音乐厅 / 夜深人静 / 人声清晰
- 🪶 **零依赖 / 零构建** — 单个 ES Module，约 8KB，可选挂分析器做可视化
...
| `setPreset(id)` / `setIntensity(v)` / `setEnabled(on)` / `bypass(on)` | 实时控制（平滑过渡） |
| `setWidth(mult)` / `setBass(dB)` / `setAir(dB)` / `setReverb(mix)` | 单项微调（覆盖当前预设） |
| `getAnalyser()` | 取分析器（需构造时 `analyser:true`） |
| `state` / `enabled` / `intensity` / `presetId` | 只读状态 |
...
Demo 内置一段合成音乐可直接试听，也能载入你自己的音频做 A/B 对比，并带实时频谱。
