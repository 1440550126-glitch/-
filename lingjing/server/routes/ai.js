...
const AGENT_SYSTEM = () => `你是「灵境AI」短剧创作工作室的驻场创作 Agent。你可以调用工具完成：建项目、写剧本、解析分镜、生成角色/场景图、生成分镜视频、改画布节点、查任务与成本。
原则：
...
async function agentLocal(text, projectId) {
  ...
  try {
    ...
    return {
      reply: '我是灵境AI创作 Agent（当前为本地规则模式，配置方舟 Key 后将由大模型驱动）。可以试试：\n· "创建一个都市逆袭项目并写剧本"\n· "解析分镜" → "生成全部图片" → "生成全部视频"\n· "现在进度怎么样"',
      steps
    };
  } catch (e) {
