  ...
  try {
    ...
    for (const act of plan.slice(0, p.maxSteps)) {
      ...
      if (act === 'generate_script') {
        ...
        const r = await run('generate_script', { project_id: pid, idea: text.slice(0, 300), genre: ent.genre, style: ent.style, num_episodes: ent.movie ? 1 : ent.episodes, num_scenes: ent.numScenes });
...
export async function analyzeIntentLLM(messages, projectId) {
...
3. 电影/长片：generate_script 传 num_episodes=1（解析会自动分幕）；短剧按用户集数；
...
export async function arkAgent(messages, projectId) {
  ...
  let analysis = null;
  if (p.planFirst) {
    try { analysis = await analyzeIntentLLM(messages, projectId); } catch (e) { console.warn('[agent] 意图分析失败：', e.message); }
  }
  const thinking = analysis?.summary ? `${analysis.summary}${analysis.plan?.length ? `\n规划：${analysis.plan.join(' → ')}` : ''}` : '';
  ...
  const msgs = [
    { role: 'system', content: EXEC_SYSTEM(plan) + (projectId ? `\n当前上下文项目 project_id：${projectId}。` : '') },
    ...messages.slice(-12)
  ];
  const steps = [];
