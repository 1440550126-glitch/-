...
import { pickImageProvider, pickVideoProvider, maxVideoDuration, supportsMultiRef, openaiImage, googleVeoCreate, googleVeoGet, dashscopeImage, dashscopeVideoCreate, dashscopeTaskGet, viduReferenceVideoCreate, viduTaskGet, klingVideoCreate, klingTaskGet, moxingVideoCreate, moxingTaskGet } from './providers.js';
...
function framingGuide(kind, shotType = '') {
  if (kind === 'character') {
    return `，角色设定板（character model sheet / 身份板）：同一角色多视角全身（正面·背面·四分之三侧面·坐姿·蹲姿·仰视·俯视）＋ 表情研究头像排 ＋ 标志配饰/手部/面部细节特写，所有视角的五官·发型·服装·配色·身高体型完全一致、严格同一人，纯白背景、无投影，五官清晰对称、表情自然不诡异（避免恐怖谷），双手结构正确（五指、不多手不畸形），外貌特征鲜明辨识度高、不撞脸。${NEG_TAIL}`;
  }
  if (kind === 'scene') {
...
{"title":"片名","logline":"一句话故事","style":"画面风格","unit":"幕或集",
 ...
 "shots":[{"key":"sh1","order":1,"episode":"e1","scene":"s1","characters":["c1"],"shot_type":"远景/全景/中景/近景/特写","camera":"运镜描述（如：固定机位/缓推/拉镜/横移/摇镜/跟随/手持抖动/升降臂，可加速度如'缓慢推进'/'快速横移'，电影感镜语优先）","emotion":"主要角色微表情(可空，优先用精确微表情名让画面更生动：喜→挑眉戏谑笑/心动羞涩笑/被夸小得意/久别重逢笑；怒→微蹙不悦/瞪眼质问/咬牙隐忍/濒临爆发；哀→轻叹失落/含泪哽咽/强颜欢笑/绝望低垂；惊→微微一愣/倒吸冷气/目瞪口呆/惊魂未定；也可填粗情绪冷酷/微笑/悲伤等)","wardrobe":"本镜特殊造型/换装(可空；剧情有换衣或时代切换时填，如'民国蓝布学生装'/'60年代蓝工装'/'现代职业装'——只换衣不换脸)","action":"画面内发生的事","dialogue":"台词(可空)","duration":"秒数(普通镜头3-8；长镜头/慢节奏可到15-30，出片时按所选模型上限自动裁剪)","image_prompt":"该镜头首帧文生图提示词，写明出场人物名与场景","video_prompt":"图生视频动态提示词（重点描述动作/表情/物理变化，不重复运镜，30字以内）"}]}
铁律：
...
2. 每个人物必须判断 gender（只能填"男"或"女"）与年龄段，写进 desc 与 image_prompt；desc 要含外貌/默认服装/性格；
2.5 锁人物＝全程一张脸：同一角色在不同场景/年代/换装时仍是同一张脸（五官·脸型·发型·身高体型不变），只把"换上的衣服/造型"填进该镜 wardrobe，绝不换人；穿越/回忆/时代切换尤其注意；
...
export function normalizeStoryboard(sb) {
  ...
  rawShots.forEach(({ sh }, i) => {
    out.shots.push({
      ...
      emotion: String(sh.emotion || '').slice(0, 10), wardrobe: String(sh.wardrobe || '').slice(0, 40),
      action: String(sh.action || '').slice(0, 200), dialogue: String(sh.dialogue || '').slice(0, 120),
      duration: clamp(sh.duration || 5, 2, 30),   // 存"意图时长"上限 30（最长模型 Seedance 2.5）；出片时再按所选模型上限裁剪
...
// 服装/造型与「脸」分离：锁人物＝全程一张脸（五官·脸型·发型·身高体型），服装可随剧情/年代/场景更换（穿越/换装剧需要）
const WARDROBE_RE = /([^，。；、\s]{0,6}(?:卡通睡衣|睡衣|学生装|工装|工作服|连衣裙|旗袍|衬衫|西装|大衣|外套|风衣|制服|校服|军装|战袍|铠甲|盔甲|长袍|马褂|短打|布衣|戎装|裙|袍|衫|装))/;
const COSTUME_CHANGE_RE = /(?:换上|穿上|身着|身穿|换成|改穿|换好|穿着|披上)([^，。；、！？!?]{2,16})/;
/** 抽取默认造型（定妆照穿什么） */
function extractWardrobe(desc) {
  const m = String(desc || '').match(WARDROBE_RE);
  return m ? m[1].replace(/^[，,。.；;、的着]+/, '') : '';
}
/** 从动作里识别本镜换装（如「换上60年代蓝工装」），返回新造型或空 */
function detectCostume(action) {
  const m = String(action || '').match(COSTUME_CHANGE_RE);
  return m ? cleanDesc(m[1]) : '';
}

// 视觉签名（VISUAL SIGNATURE）：从描述里蒸馏 2-6 个"必出"识别特征（发型/头饰/眼镜/面部标记/配饰/标志服饰/鞋），
// 作为最强一致性锚点，逐字写进锁定档案与每个分镜（业内"身份板"核心：限定角色细节）。
const SIG_PATTERNS = [
  /(卷发|直发|短发|长发|马尾|双马尾|丸子头|齐刘海|寸头|波浪发|盘发|光头|板寸|爆炸头|麻花辫|公主切|银发|白发|金发)/,
  /(头巾|发带|发箍|头花|草帽|礼帽|斗笠|皇冠|发夹|缎带|头盔|面纱)/,
  /(墨镜|眼镜|框镜|单片镜|护目镜|面具|眼罩)/,
  /(络腮胡|山羊胡|胡须|刀疤|疤痕|泪痣|美人痣|雀斑|酒窝|纹身|胎记|独眼)/,
  /(项链|吊坠|耳环|耳钉|手链|手镯|戒指|围巾|领带|领结|徽章|怀表|挂坠|护符|腰带)/,
  /([^，。；、\s]{0,3}(?:衬衫|卫衣|风衣|大衣|西装|旗袍|连衣裙|工装|铠甲|战袍|长袍|马甲|背心|牛仔|皮衣|制服|校服|和服|斗篷))/,
  /(西部靴|马丁靴|高跟鞋|运动鞋|皮鞋|长靴|草鞋|布鞋|战靴)/
];
function visualSignature(desc) {
  const d = String(desc || '');
  const out = [];
  for (const re of SIG_PATTERNS) {
    const m = d.match(re);
    if (m) { const t = m[1].replace(/^[，,。.；;、的]+/, ''); if (t && !out.includes(t)) out.push(t); }
  }
...
function buildLocks(sb) {
  ...
  for (const c of sb.characters) {
    ...
    c.wardrobe = extractWardrobe(d);   // 默认造型（定妆照穿什么）
    c.signature = visualSignature(d);  // 视觉签名（必出识别特征）：最强一致性锚点
    const sigTxt = c.signature ? `；视觉签名(必出特征)：${c.signature}` : '';
    // 锁人物＝全程一张脸：五官·脸型·发型·身高体型固定不变、严禁换人；服装造型可随剧情/年代/场景更换（穿越/换装也保持同一张脸）
    c.lock = `${c.name}（${g}${age}/${c.role}：${d}${body ? '，' + body : ''}，五官·脸型·发型·身高体型比例全程固定不变、自始至终同一张脸严禁换人；服装造型可随剧情场景更换，仅换装不换脸${sigTxt}）`;
    // 角色设定板（身份板）＝全片定海神针参考：结构化标注 + 多视角多姿态 + 表情研究 + 细节研究（含三视图），复杂景别运动下仍锁住角色
    c.image_prompt = `${style} ｜ 角色设定板 character model sheet（含三视图＋多姿态＋表情研究＋细节研究） ｜ 顶部标注：角色名「${c.name}」/身份「${c.role}」${c.signature ? `/视觉签名「${c.signature}」` : ''} ｜ 多视角全身：正面·背面·四分之三侧面·坐姿·蹲姿·仰视·俯视，所有视角同一人五官发型完全一致 ｜ 表情研究：冷漠·微笑·惊讶·愤怒·悲伤 头像一排 ｜ 细节研究：面部·发型·标志配饰·手部 特写 ｜ ${c.lock} ｜ 纯白背景、角色设定表排版、面部五官比例真实、自然透视无畸变、各视角严格同一人`;
  }
...
const MOVE_RULES = [
  ...
  // 单字运镜兜底（分镜表常写 推/拉/移 单字）；放最后，前面更具体的（快推/横移/拉镜）优先命中
  [/推/, '缓慢推进（dolly-in）'],
  [/拉/, '拉镜（dolly-out）'],
  [/移/, '横向跟踪移动（tracking shot）'],
];
...
function buildBible(sb) {
  ...
  return `【全片总控】影像风格：${styleAnchor(sb)}。【画风铁律】全片只用这一种画风，${STYLE_DRIFT_NEG}【角色档案锁定】${chars}。【铁律】锁人物＝全程一张脸：各角色五官·脸型·发型·身材比例全片严格一致、自始至终同一个人，不同角色明显区分不可撞脸、不得中途换人；服装造型可随剧情/年代/场景更换（穿越或换装时仍是同一张脸，只换衣不换人）。`;
}
...
function alignStoryboard(sb) {
  ...
  for (const sh of sb.shots) {
    ...
    // 本镜造型：显式 wardrobe > 动作里识别的换装（如「换上60年代蓝工装」）；有则覆盖默认造型，但脸不变
    const shotWardrobe = cleanDesc(sh.wardrobe) || detectCostume(sh.action);
    const wardrobeNote = shotWardrobe ? `【本镜造型】${chars[0]?.name || '主角'}：${shotWardrobe}（仅换装/换造型，脸型·五官·发型·身高体型不变，仍是同一人）` : '';
    // 规范化首帧提示词：画风锚点 + 景别 + 摄影机参数 + 角色锁定档案 + 本镜造型 + 场景锁定 + 画面动作 + 情绪
    const parts = [style, sh.shot_type || '中景', `【摄影机】${cinema}`];
    if (chars.length) parts.push(`【角色】${chars.map((c) => c.lock).join('；')}`);
    if (wardrobeNote) parts.push(wardrobeNote);
    ...
    if (sh.emotion) parts.push(`【微表情】${chars[0]?.name || '主角'}：${resolveExpression(sh.emotion)}`);
    sh.image_prompt = parts.join(' ｜ ').slice(0, 1100);
    // 视频提示词：动作 + 摄影机参数 + 微表情 + 该镜角色锁定档案（脸全程不变）+ 本镜造型（可换装）
    const vp = cleanDesc(sh.video_prompt).slice(0, 70) || action;
    const head = `${vp}｜【摄影机】${cinema}${sh.emotion ? `｜【微表情】${resolveExpression(sh.emotion)}` : ''}`;   // 动作+摄影机+微表情优先保留，锁定档案放尾部
    const lockLine = chars.length ? `｜【角色锁定】${chars.map((c) => c.lock).join('；')}` : '｜角色外观全程一致';
    sh.video_prompt = (head + (wardrobeNote ? `｜${wardrobeNote}` : '') + lockLine).slice(0, 720);
  }
}
...
export async function generateExpressions({ projectId, nodeId, emotions = [] }) {
  ...
  for (const emo of list) {
    const r = await generateImage({
      prompt: `${node.data.prompt || node.data.desc || node.data.name}，微表情【${emo}】：${resolveExpression(emo)}，与参考图为同一个人（五官、发型、脸型、肤色、服装完全一致），仅表情变化，单人正脸表情定妆照，五官清晰对称、表情自然真实、不变形不诡异、避免恐怖谷，纯色背景`,
...
export function buildOmniReferencePrompt(projectId, { episode = '' } = {}) {
  ...
  shots.forEach((sh, i) => {
    ...
    lines.push(`镜头${i + 1}（${clamp(sh.duration || 5, 2, 30)}秒）：${sh.shot_type || '中景'}｜${cinema}｜${light}。${who}${where ? where : ''}${act ? '，' + act : ''}${say ? `，说道："${say}"` : ''}。`);
  });
  return {
...
export function buildCharacterProfile(projectId) {
  ...
  const characters = (sb.characters || []).map((ch) => {
    ...
    return {
      ...
      visual_signature: ch.signature || visualSignature(cleanDesc(ch.desc)),   // 必出识别特征（身份板核心锚点）
      default_wardrobe: ch.wardrobe || extractWardrobe(cleanDesc(ch.desc)),    // 默认造型（可随剧情换装）
      lock: ch.lock || '',                       // 全片每处逐字复用的锁定档案
      portrait,                                  // 角色设定板/定妆照（首帧参考基准），空＝未生成
...
export async function createVideoTask({ prompt, imageUrl = '', lastImageUrl = '', refImages = [], duration = 5, ratio = '', projectId = '', nodeId = '', name = '', order = 0, model = '', resolution = '' }) {
  ...
  const provLabel = vp.provider === 'google' ? 'Veo' : vp.provider === 'vidu' ? 'Vidu 全能参考' : vp.provider === 'kling' ? '可灵 Kling' : vp.provider === 'moxing' ? '墨行AI' : vp.provider === 'alibaba' ? '通义万相' : '方舟视频';
    ...
    try {
      const r = vp.provider === 'google'
        ...
        : vp.provider === 'vidu'
          ...
          : vp.provider === 'kling'
            ? await klingVideoCreate({ prompt, imageUrl, images: refList, ratio, duration, model: vp.model })
            : vp.provider === 'moxing'
              ? await moxingVideoCreate({ prompt, imageUrl, images: refList, ratio, duration, model: vp.model })
              : vp.provider === 'alibaba'
...
export async function pollTask(taskId) {
  ...
  if (['ark', 'google', 'alibaba', 'vidu', 'kling', 'moxing'].includes(t.provider) && t.remote_id) {
    try {
      const r = t.provider === 'google'
        ...
        : t.provider === 'vidu'
          ...
          : t.provider === 'kling'
            ? await klingTaskGet(t.remote_id, { duration: params.duration || 5 })
            : t.provider === 'moxing'
              ? await moxingTaskGet(t.remote_id, { duration: params.duration || 5 })
              : t.provider === 'alibaba'
      ...
      const label = { google: 'Veo', vidu: 'Vidu', kling: '可灵', moxing: '墨行AI', alibaba: '通义万相' }[t.provider] || '方舟';
