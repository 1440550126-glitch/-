...
{"title":"片名","logline":"一句话故事","style":"画面风格","unit":"幕或集",
 ...
 "shots":[{"key":"sh1","order":1,"episode":"e1","scene":"s1","characters":["c1"],"shot_type":"远景/全景/中景/近景/特写","camera":"运镜描述（如：固定机位/缓推/拉镜/横移/摇镜/跟随/手持抖动/升降臂，可加速度如'缓慢推进'/'快速横移'，电影感镜语优先）","emotion":"主要角色情绪(可空：冷酷/愤怒/狂喜/悲伤/微笑/惊恐/魅惑/羞涩)","wardrobe":"本镜特殊造型/换装(可空；剧情有换衣或时代切换时填，如'民国蓝布学生装'/'60年代蓝工装'/'现代职业装'——只换衣不换脸)","action":"画面内发生的事","dialogue":"台词(可空)","duration":4,"image_prompt":"该镜头首帧文生图提示词，写明出场人物名与场景","video_prompt":"图生视频动态提示词（重点描述动作/表情/物理变化，不重复运镜，30字以内）"}]}
铁律：
...
export function normalizeStoryboard(sb) {
  ...
  rawShots.forEach(({ sh }, i) => {
    out.shots.push({
      ...
      emotion: String(sh.emotion || '').slice(0, 6), wardrobe: String(sh.wardrobe || '').slice(0, 40),
...
function alignStoryboard(sb) {
  ...
  for (const sh of sb.shots) {
    ...
    // 视频提示词：动作 + 摄影机参数 + 该镜角色锁定档案（脸全程不变）+ 本镜造型（可换装）
    ...
    sh.video_prompt = (head + (wardrobeNote ? `｜${wardrobeNote}` : '') + lockLine).slice(0, 640);
  }
}
