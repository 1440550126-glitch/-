...
import { openEntityReview } from '../entities.js';
...
export async function renderProject(page, params) {
  ...
  qcBtn.innerHTML = `🔬 QC`;
  const entBtn = h('button', { class: 'btn', title: '角色预选标注：校验谁是角色/场景/道具，校正即训练 Agent 进化', onclick: () => {
    if (!project.storyboard) return toast('先解析剧本', 'err');
    openEntityReview({ projectId: project.id, onApplied: () => reload(true) });
  } });
  entBtn.innerHTML = `${icon('user', 15)} 校验角色`;

  // 全能参考一键出片：角色三视图+场景图作多图参考，模型跟随顶栏「视频模型」（Vidu/可灵 真·多主体参考）
  const omniBtn = h('button', { class: 'btn', title: '全能参考：用角色三视图+场景图作多图参考，一次产出人物/场景一致的连续短片。模型跟随顶栏「视频模型」——选 Vidu 或可灵才是真·多主体参考', onclick: async () => {
    if (!project.storyboard) return toast('先解析剧本', 'err');
    omniBtn.disabled = true;
    const old = omniBtn.innerHTML;
    omniBtn.innerHTML = `${icon('loader', 15)} 全能参考出片中…`;
    try {
      const r = await POST(`/api/projects/${project.id}/omni-video`, { model: vidModelSel.value || '' });
      if (r.missing_images?.length) toast(`${r.missing_images.length} 个参考图还没生成，建议先出图再用全能参考`, 'err');
      if (r.note) toast(r.note, 'err');
      toast(`已提交全能参考出片（${r.model}，带入 ${r.used_images} 张参考图），生成中…`, 'ok');
  ...
  async function doParse() {
    ...
    try {
      ...
      await reload();
      // 解析完自动弹出"角色预选"复核：用户校验分类，校正即训练 Agent
      openEntityReview({ projectId: project.id, onApplied: () => reload(true) });
    } catch (e) { toast(e.message, 'err'); }
