...
npm run studio:smoke    # 108 项全链路冒烟测试（API + Agent + MCP stdio/HTTP）
```
...
| | 小云雀（剪映） | 灵境AI |
|---|---|---|
| 创作流程 | 剧本 → 资产 → 画布 → 成片 | 一样齐全（AI 生剧本：**电影长片(六幕)/短剧多集** / 粘贴剧本 / 自由画布），另有**全流程工作流**：一键托管 剧本→解析→体检→出图→出片→配音→导出，实时步骤进度、可取消、未配置步骤自动跳过（Agent 有 run_workflow/get_workflow 工具） |
...
| 剧本解析 | 自动拆角色/场景 | **长片分幕分段送模型**避免截断；角色识别**自动判别男女**、过滤旁白/字幕等非人物；电影自动分幕、场景/分镜随体量放大 |
...
| Agent 接入 | 会员专属 Skill（`npx @pippit-dev/cli`，闭源） | **MCP + OpenAPI + 内置 Agent 三通道，全开放免费**；内置 Agent v2 会**先分析意图、规划步骤再智能调度**（温度/步数/自动执行等参数可调） |
