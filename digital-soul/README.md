...
## 它早已不只是"对话"——一个会陪伴、会守护、像人一样思考的存在

下面这些**张口就能用**（不接大模型也能跑，接了 Ollama 更自然）。括号里是配置档，补得越全越像本人。

### 🧠 像人一样思考（先理解，再回应）
- 听话听音：你说「我没事」，它不信字面 → "我瞧着你不太对劲，别自己扛着"
- 开口前先思量（是谁→言外之意→想起啥→自己情绪→怎么接，网页"🧠 刚才怎么想的"可见）
- 会掂量会转念：「该不该辞职」→ 初念→转念想另一面→连上三观和过往→落定，把主意留给你
- 听不懂就老实问、像人一样追问（"哦？玩得开心吧？"）
- 看出门道：你这几天反复提睡不好 → 主动点破"是有心事吧？"
- 越处越懂：把察觉沉淀成「我眼里的你」；还会**主动牵挂**——"小婷有5天没见，上回你说睡不好，好些没？"

### 🫂 陪伴（present-tense，绝不吓人）
- 怕黑/做噩梦/独自在家 → 立刻给安全感；担忧 → 顺气重构；和家人吵架 → 当和事佬
- **老来的宽慰**：怕成累赘/没用了/记性差 → "你是这个家的根，忘事有我替你记着"
- 报喜道贺、考前打气（事后第二天主动追问）、攒小确幸、陪聊、一起回忆、走心夸夸
- 玩游戏（猜谜/成语接龙）、解闷、唱老歌、讲睡前故事（`config/bedtime.yaml`/`music.yaml`）

### 🩺 守护（像家里最上心的那个人）
- 用药（`config/medications.yaml`：几点吃、吃了没、还剩多少、该续配）
- **应急**（摔了/胸口闷/喘不上气 → 稳住+指引+报联系人，`config/contacts.yaml`）
- **体征**（"血压145 92"就记一笔，看趋势、异常提醒）· 就医提醒 · 睡前安全检查
- 习惯打卡 · 今天吃啥 · 穿衣带伞 · 养生操 · 节气养生 · 养花浇水 · 常联系亲友

### 💑 夫妻 / 👪 全家
- 老伴专属（`config/spouse.yaml`）：昵称、我们的故事、纪念日、思念陪伴、哄人、晚安、**情书**、日常默契
- **捎话**："等小明回来跟他说妈喊吃饭"，等人来了主动捎到
- **家庭分工板**："今天小婷买菜"，谁来了提醒谁 · 家庭账本 · 家珍册（一页传家）

### 👤 灵魂 / 性格
- 自己的口味脾性（`config/preferences.yaml`）· 对人生话题的一贯主见（`config/opinions.yaml`）
- 会讲段子俏皮回嘴（`config/humor.yaml`）· 一肚子应景老话 · 没人时心里也惦记着家人

> 全部本机生成，不联网、不替任何人登录任何系统。一句"说说今天"，它能把天气穿衣/吃药/就医/生日/养生/浇花/分工汇成一段；周末一句"这周怎么样"，陪你把日子回望一遍。
...
| 语音交流（听 + 说） | 本地 Whisper 转文字 + TTS（**Mac 的 say / Linux 的 espeak 零安装即出声**，也支持 pyttsx3 / 声音克隆），语速/音量随七情变化 | `dsoul/voice.py` · `scripts/voice_chat.py` |
...
| 采买清单 | "买瓶酱油""鸡蛋买好了"，分身替你记着购物清单 | `dsoul/shopping.py` |
| 哀伤阶段陪伴 | 按"离开多久"分阶段(初痛/浓念/渐和/长念)，给贴合此刻的安慰 | `dsoul/comfort_stages.py` |
